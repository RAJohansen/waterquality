...
```
```{r}
extract_lm_results <- extract_lm(parameter = "Chl_ugL", algorithm = "MM12NDCI", df = df)
dplyr::arrange(extract_lm_results,desc("R_Squared"))
```
...
```{r}
extract_lm_cv_results <- extract_lm_cv(parameter = "Chl_ugL", algorithm = "MM12NDCI", df = df, train_method = "lm", control_method = "repeatedcv", folds = 3, nrepeats = 5)
head(extract_lm_cv_results)
```
...
extract_lm_cv_multi_results <- extract_lm_cv_multi(parameter = parameters, algorithm = algorithms, df = df, train_method = "lm", control_method = "repeatedcv", folds = 3, nrepeats = 5)
...
```{r}
extract_lm_cv_all_results <- extract_lm_cv_multi(algorithm = algorithms, df = df, train_method = "lm", control_method = "repeatedcv", folds = 3, nrepeats = 5)

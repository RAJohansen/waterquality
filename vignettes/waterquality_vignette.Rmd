...
Currently, we have a list of [45 algorithms](https://rajohansen.github.io/waterquality/reference/index.html) that can be applied for the detection of these three parameters. 
...
The final outputs are raster stacked images which represent relative index values and are not direct estimated chlorophyll-a, phycocyanin, or turbidity values. However, these relative values can be converted in to estimated concentration values using the `extract_lm` functions and ground truth measurements. 
...
### Map_WQ_raster
This function wraps the ["tmap"](https://cran.r-project.org/web/packages/tmap/tmap.pdf) package to help users to efficiently generate a map of a raster image which can be overlaid with optional geospatial objects and data histogram. In order to simplify this process and reduce the technical expertise required, we have reduced the number of arguments to the following: 
...
## Modeling Functions
Developing models is an essential step in water quality monitoring, but this does require sufficient coincident ground truth data. These functions have been developed to combine user-provided truth data and the results from the `wq_calc` function to generate a single or series of regression models. These functions have been developed to easily conduct linear models with standardized outputs. These functions are meant intuitive yet robust. Most effectively the functions will utilize a single comma delimited (.csv) files as the data frame source. Although these functions were designed for our water quality research, they can be used as standalone functions for any data to generate the same standardized outputs containing the following:   
**Global Model**  
- r^2^  
- p-value  
- slope  
...
Additionally we have provided code that might be useful to help users import and extract the data from a raster stack image (i.e. output from `wq_calc`), combine it with a geospatial object (i.e shapefiles of sampling locations), and export the results to a .csv file to be used in the modeling functions. 
